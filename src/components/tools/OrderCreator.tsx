...
import { 
  ...
  Upload, Image as ImageIcon, CreditCard, Percent, Truck
} from "lucide-react";
...
// --- COMPONENTE VISUAL DO PEDIDO (PRINT) ---
const OrderTemplate = ({ data }: { data: OrderData }) => {
    ...
    const total = Math.max(0, subtotal - discountValue + (data.shipping.value || 0));
        ...
        <div className="w-full h-full bg-white p-8 md:p-12 text-slate-900 font-sans relative flex flex-col justify-between">
            ...
            <div className="flex justify-between items-start border-b-2 border-slate-100 pb-8 mb-8">
                ...
                <div className="text-right">
                    <div className="inline-block bg-slate-100 px-4 py-2 rounded-lg mb-2">
                        <h2 className="text-xs font-bold uppercase tracking-widest text-slate-500">Pedido Nº</h2>
                    ...
                    {data.deliveryDate && <p className="text-sm text-slate-500">Entrega: <strong>{data.deliveryDate}</strong></p>}
                </div>
            </div>
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="border-b-2 border-slate-900">
                            <th className="py-3 text-xs font-bold uppercase tracking-wider text-slate-500 w-[50%]">Produto / Serviço</th>
            ...
            <div className="flex flex-col sm:flex-row justify-between mt-8 gap-8">
                ...
                <div className="w-full sm:w-1/2 space-y-6">
                    ...
                    {(data.shipping.value > 0 || data.shipping.notes) && (
                        <div>
                             <h4 className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-3 border-b border-slate-100 pb-1">Frete / Entrega</h4>
                             <div className="text-sm text-slate-600 space-y-1">
                                {data.shipping.value > 0 && <p><strong>Valor:</strong> {formatCurrency(data.shipping.value)}</p>}
                                {data.shipping.notes && <p className="italic">{data.shipping.notes}</p>}
                             </div>
                        </div>
                    )}
                </div>
            ...
            <div className="mt-12 pt-8 border-t border-slate-200">
                {data.terms && (
                    <div className="mb-8">
                        <h4 className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-2">Observações</h4>
...
export default function OrderCreator({ initialValues }: OrderCreatorProps = {}) {
    // --- ESTADO ---
    ...
    const [orderNumber, setOrderNumber] = useState(initialValues?.number || "0001");
    ...
    const [items, setItems] = useState<OrderItem[]>(initialValues?.items || [
        { id: '1', description: 'Produto Exemplo', quantity: 1, price: 100.00 }
    ]);
    
    // Desconto, Frete e Pagamento
    ...
    const [shippingValue, setShippingValue] = useState<number>(initialValues?.shipping?.value || 0);
    const [shippingNotes, setShippingNotes] = useState(initialValues?.shipping?.notes || "");
    ...
    const [terms, setTerms] = useState(initialValues?.terms || "1. Garantia de 30 dias.\n2. Trocas somente com defeito de fabricação.");
    ...
    const handlePrint = useReactToPrint({
        contentRef: printRef,
        documentTitle: `Pedido_${orderNumber}_${clientName || 'Cliente'}`,
    ...
    const orderData: OrderData = {
        ...
        shipping: { value: shippingValue, notes: shippingNotes },
    ...
    const calculateTotal = () => {
        ...
        const total = Math.max(0, sub - disc + (shippingValue || 0));
        return total;
    };
            ...
            <div className="w-full space-y-6">
                    ...
                    <CardContent className="p-6 grid md:grid-cols-2 gap-6">
                         <div className="space-y-4">
                            <div className="space-y-2">
                                ...
                                <div className="flex items-center gap-4 mt-2">
                                    ...
                                    <div className="relative">
                                        ...
                                        <Button variant="outline" size="sm" asChild className="cursor-pointer border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800">
                            ...
                            <div className="space-y-2">
                                ...
                                <Input placeholder="Sua Loja" value={issuerName} onChange={e => setIssuerName(e.target.value)} className="dark:bg-slate-800 dark:border-slate-700 dark:text-white" />
                            </div>
                         </div>
                         <div className="space-y-4">
                            <div className="space-y-2">
                            ...
                            <div className="space-y-2">
                                ...
                                <Input placeholder="Rua Loja, 123 - Cidade/UF" value={issuerAddress} onChange={e => setIssuerAddress(e.target.value)} className="dark:bg-slate-800 dark:border-slate-700 dark:text-white" />
                            </div>
                         </div>
                    </CardContent>
                </Card>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                    <Card className="border-0 shadow-sm ring-1 ring-slate-200 dark:ring-slate-800 bg-white dark:bg-slate-900">
                        ...
                        <CardContent className="p-6 space-y-3">
                            <div className="space-y-2">
                                ...
                                <Input placeholder="Maria Souza" value={clientName} onChange={e => setClientName(e.target.value)} className="dark:bg-slate-800 dark:border-slate-700 dark:text-white" />
                            </div>
                            <div className="space-y-2">
                            ...
                            <div className="space-y-2">
                                <Label className="dark:text-slate-300">Endereço de Entrega</Label>
                                <Input placeholder="Endereço de entrega" value={clientAddress} onChange={e => setClientAddress(e.target.value)} className="dark:bg-slate-800 dark:border-slate-700 dark:text-white" />
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    <Card className="border-0 shadow-sm ring-1 ring-slate-200 dark:ring-slate-800 bg-white dark:bg-slate-900">
                        ...
                        <CardContent className="p-6 space-y-3">
                            <div className="space-y-2">
                                <Label className="dark:text-slate-300">Número do Pedido</Label>
                                <Input value={orderNumber} onChange={e => setOrderNumber(e.target.value)} className="dark:bg-slate-800 dark:border-slate-700 dark:text-white" />
                            </div>
                            <div className="grid grid-cols-2 gap-3">
                                <div className="space-y-2">
                                    <Label className="dark:text-slate-300">Data do Pedido</Label>
                                    <Input value={orderDate} onChange={e => setOrderDate(e.target.value)} className="dark:bg-slate-800 dark:border-slate-700 dark:text-white" />
                                </div>
                                <div className="space-y-2">
                                    <Label className="dark:text-slate-300">Previsão Entrega</Label>
                                    <Input value={deliveryDate} onChange={e => setDeliveryDate(e.target.value)} className="dark:bg-slate-800 dark:border-slate-700 dark:text-white" />
                                </div>
                            </div>
                        </CardContent>
                    </Card>
                ...
                <Card className="border-0 shadow-sm ring-1 ring-slate-200 dark:ring-slate-800 bg-white dark:bg-slate-900">
                     <CardHeader className="bg-slate-50 dark:bg-slate-950/50 border-b border-slate-100 dark:border-slate-800 pb-4 flex flex-row items-center justify-between">
                        ...
                        <Button onClick={addItem} size="sm" className="bg-slate-900 dark:bg-slate-800 text-white hover:bg-slate-800 dark:hover:bg-slate-700">
                     ...
                     <Card className="border-0 shadow-sm ring-1 ring-slate-200 dark:ring-slate-800 bg-white dark:bg-slate-900">
                        ...
                        <CardContent className="p-6 space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="space-y-2">
                                    <Label className="dark:text-slate-300">Obs. Pagamento</Label>
                                    <Input placeholder="Ex: Entrada + 1x" value={paymentNotes} onChange={e => setPaymentNotes(e.target.value)} className="dark:bg-slate-800 dark:border-slate-700 dark:text-white" />
                                </div>
                            </div>

                            <div className="space-y-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                                <Label className="flex items-center gap-2 dark:text-slate-300"><Truck size={14}/> Frete / Entrega (R$)</Label>
                                <div className="grid grid-cols-3 gap-2">
                                    <div className="col-span-1">
                                         <Input 
                                            type="text"
                                            inputMode="numeric"
                                            value={formatCurrencyInput(shippingValue)} 
                                            onChange={e => setShippingValue(parseCurrency(e.target.value))}
                                            placeholder="Valor" 
                                            className="dark:bg-slate-800 dark:border-slate-700 dark:text-white"
                                        />
                                    </div>
                                    <div className="col-span-2">
                                        <Input 
                                            placeholder="Obs: Correios, Motoboy..." 
                                            value={shippingNotes} 
                            ...
                            <div className="pt-2 border-t border-slate-100 dark:border-slate-800">
                                <Label className="mb-2 block dark:text-slate-300">Desconto</Label>
                ...
                <Card className="border-0 shadow-sm ring-1 ring-slate-200 dark:ring-slate-800 bg-white dark:bg-slate-900">
                    ...
                    <CardContent className="p-6">
                        <Textarea 
                            ...
                            placeholder="Descreva a garantia, política de troca ou agradecimentos..."
                        />
                    </CardContent>
                </Card>
                ...
                <div className="flex gap-4 sticky bottom-4 z-50 bg-white/80 dark:bg-slate-900/80 p-4 rounded-2xl shadow-xl backdrop-blur-md border border-slate-200/60 dark:border-slate-700/60 max-w-2xl mx-auto">
                    ...
                    <Button 
                        size="lg" 
                        onClick={() => handlePrint()} 
                        className="flex-1 bg-green-600 hover:bg-green-700 text-white font-bold shadow-green-200 dark:shadow-none shadow-lg"
                    >
                        <Printer size={20} className="mr-2"/> Imprimir Pedido
                    </Button>
                </div>
             ...
             {/* --- PREVIEW ÁREA --- */}
            <div ref={previewRef} className="w-full bg-slate-800 p-4 md:p-12 rounded-3xl border border-slate-700 flex justify-center shadow-inner">
